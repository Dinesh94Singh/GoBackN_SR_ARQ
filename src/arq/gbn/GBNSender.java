...
public class GBNSender extends ARQSocket implements Runnable {
		...
		while (currentPacketIndex < dataPackets.length) {
			for (int i = currentPacketIndex; i < currentPacketIndex
				...
				if (percent > packetLossRate) {
					writeln("#"+i%windowSize+":"+dataPackets[i]);
					System.out.println("#"+i%windowSize+":"+dataPackets[i] + "...Sent");
				} else {
					System.out.println("#"+i%windowSize+":"+dataPackets[i]
							+ "...P.L.R");
				}
